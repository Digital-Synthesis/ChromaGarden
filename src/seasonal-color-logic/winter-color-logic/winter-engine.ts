...
export type WinterRecipeDebug = {
  personalityId: string;
  label: string;
  pass: number; // ← add this line
  base: {
    exposure: number;
    contrast: number;
    temperature: number;
...
export function applyWinterHybridEngine(
  originalImageRef: React.RefObject<HTMLImageElement | null>,
  setImageSrc: (src: string | null) => void,
  recipe: WinterRecipeDebug
) {
  ...
  const brightnessFactor = 1 + exposure; // exposure ~ -0.3..+0.3
  const contrastFactor = 1 + contrast; // contrast ~ -0.3..+0.6
  const saturateFactor = 1 + saturation / 40; // saturation small ints
  const tempScale = temperature / 60; // winter temps ~ -40..+20
  const tintScale = tint / 60; // tint ~ -20..+30
